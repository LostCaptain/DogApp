...
total_face_in_human, total_face_in_dog = 0,0
for i in range(100):
    if(face_detector(human_files_short[i])):
        total_face_in_human += 1
    if(face_detector(dog_files_short[i])):
        total_face_in_dog += 1
print( total_face_in_human)
print( total_face_in_dog)
...
# 
# __回答:__不合理。在训练人脸识别的神经网络时加入面部特征模糊的训练集。缩小过滤器尺寸和步长。
# 
# 
...
total_dog_in_human, total_dog_in_dog = 0,0
for i in range(100):
    if(dog_detector(human_files_short[i])):
        total_dog_in_human += 1
    if(dog_detector(dog_files_short[i])):
        total_dog_in_dog += 1
print( total_dog_in_human)
print( total_dog_in_dog)
...
# 
# __回答:__ 上图所示卷积网络使用较小了的过滤器尺寸，多个小尺寸过滤器和一个大尺寸的过滤器都能达到识别大尺寸特征的效果，只是小尺寸的过滤器运算会大大降低了参数的个数以及计算复杂度。在卷积层后面接了一个最大池化层的作用是降低采样，保留显著特征，降低特征维度。在卷积网络后接了一个全局平均池化层来代替全连接层是因为全连接层参数过多，运算量大，训练时间长，而且容易过拟合，而GAP相比全连接层，特征的指向性更明确。
...
train_R50 = bottleneck_features['train']
valid_R50 = bottleneck_features['valid']
test_R50 = bottleneck_features['test']
...
# 
# 
# __回答:__ 第一步：迁移使用Resnet50的预处理模型，作用是通过已经预处理好的Resnet模型加快训练速度。
#        第二步：设置全局平均池化层，作用是降低维度，减少全连接的参数。
#        第三步：激活函数。
#        由于我们需要训练的对狗的识别和Nesnet50模型所处理的问题有重叠，而该模型已经预先训练好了对很多物体的识别，包括狗，与我们的数据和任务有        相同和相关性，通过迁移学习可以将已经学到的模型参数分享给新模型从而加快并优化模型的学习效率不用像大多数网络那样从零学习。
# 

# In[16]:

### TODO: 定义你的框架
R50_model = Sequential()
R50_model.add(GlobalAveragePooling2D(input_shape=train_R50.shape[1:]))
R50_model.add(Dense(133, activation='softmax'))
...
### TODO: 训练模型
checkpointer = ModelCheckpoint(filepath='saved_models/weights.best.R50.hdf5', 
                               verbose=1, save_best_only=True)

R50_model.fit(train_R50, train_targets, 
          validation_data=(valid_R50, valid_targets),
          epochs=20, batch_size=20, callbacks=[checkpointer], verbose=1)
...
R50_model.load_weights('saved_models/weights.best.R50.hdf5')
...
### TODO: 在测试集上计算分类准确率
# 获取测试数据集中每一个图像所预测的狗品种的index
R50_predictions = [np.argmax(R50_model.predict(np.expand_dims(feature, axis=0))) for feature in test_R50]
...
### 然后返回此模型所预测的狗的品种
#from extract_bottleneck_features import *
def R50_predict_breed(img_path):
    # 提取bottleneck特征
    bottleneck_feature = extract_Resnet50(path_to_tensor(img_path))
    # 获取预测向量
    predicted_vector = R50_model.predict(bottleneck_feature)
    # 返回此模型预测的狗的品种
    return dog_names[np.argmax(predicted_vector)]
...
def Image_detector(img_path):
    if(face_detector(img_path) or dog_detector(img_path)):
        return R50_predict_breed(img_path)
    return "error"
...
#   对人脸的识别较差
# 2. 提出至少三点改进你的模型的想法。
#   也许我需要一个更好的对人脸识别的神经网络
#   当只有半张人脸的时候（如我的测试图cf1b9d16fdfaaf515b565783895494eef01f7a51.jpg），识别效果不好，也许我需要对新的人脸识别网络的数据进行数据增强，训练更多偏移和部分的人脸数据。
#   同样，我也可以对狗的训练集进行数据增强，得到更多偏移或者部分的狗的数据。

# In[26]:

## TODO: 在你的电脑上，在步骤6中，至少在6张图片上运行你的算法。
## 自由地使用所需的代码单元数吧
my_files,_ = load_dataset('Images')
print(my_files)
for files in my_files:
    print(Image_detector(files))
